/* Este é meu projeto de um adaptador para fazer com que o Shifter do Logitech G27
   Funcione no Computador pelo USB e no Xbox One pelo Thrustmaster TX Base
   Os links a seguir são as fontes de pesquisa deste projeto
   This is my project of adapter to make the Logitech G27 Shifter
   Run on Computer by USB and on Xbox One by Thrustmaster TX Base
   The following links are the research sources for this project
   https://www.racingsimulator.es/arduino-logitech-g27-shifter-a-usb/
   https://www.isrtv.com/forums/topic/24532-gearbox-connector-on-base/
   Autor Makeyne from Brazil
   Ps. Thanks Steve
 */
...
Base connector / Arduino pin LEONARDO
DIN6_1 /  nothing      /
...
|   4 | Orange   | Orange  |       3 | Shifter X axis          | pin 8  (A0) |
|   5 | White    | White   |       2 | SPI input               |             |
|   6 | Black    | Black   |       8 | GND                     | GND         |
|   7 | Red      | Red     |       6 | +5V                     | VCC         |
|   8 | Green    | Green   |       4 | Shifter Y axis          | pin 9 (A1)  |
|   9 | Red      | Red     |       1 | +5V                     | VCC         |
*/
...
void getButtonStates(int *ret)
{
    ...
    for (int i = 0; i < 16; ++i)
    {                                         // Iteration over both 8 bit registers
...
int getCurrentGear(int shifterPosition[], int btns[])
{
    ...
    if (SEQUENCIAL == 0)
    {
        ...
        {
            if (y > SHIFTER_YAXIS_135)
                gear = 1; // 1st gear
            if (y < SHIFTER_YAXIS_246)
                gear = 2; // 2nd gear
        }
        ...
        if (gear != 6)
            btns[BUTTON_REVERSE] = 0; // Reverse gear is allowed only on 6th gear position
        if (btns[BUTTON_REVERSE] == 1)
            gear = 7; // Reverse is 7th gear (for the sake of argument)

        // Envia comandos para a base do Thrustmaster TX no modo H Shifter
        // Send commands to Thrustmaster TX base in H Shifter mode
...
void loop()
{
    ...
    // ATIVA OU DESATIVA SEQUENCIAL SHIFTER PRESSIONANDO OS BOTÕES BUTTON_RED_LEFT + BUTTON_RED_RIGHT
    // ENABLED OR DISABLED SEQUENCIAL SHIFTER PRESSING THE BUTTONS BUTTON_RED_LEFT + BUTTON_RED_RIGHT
    if (SEQUENCIAL == 0 && buttonStates[7] == 1 && buttonStates[6] == 1)
    {
        SEQUENCIAL = 1;
        delay(500);
    }
    else if (SEQUENCIAL == 1 && buttonStates[7] == 1 && buttonStates[6] == 1)
    {
        SEQUENCIAL = 0;
        delay(500);
    }

    // SE O SEQUENCIAL ESTIVER LIGADO ESQUERDA E DIREITA FUNCIONAM COMO CAMBIO SEQUENCIAL
    // IF THE SEQUENCIAL IS CONNECTED LEFT AND RIGHT WORK AS A SEQUENTIAL SHIFTER
    if (SEQUENCIAL == 1 && buttonStates[16] < 320)
    {
        G27.setButton(19, HIGH);
        setHMode(false);
        switchSGear(down);
        sendCommand();
    }
    else if (SEQUENCIAL == 1 && buttonStates[16] > 680)
    {
        G27.setButton(20, HIGH);
        setHMode(false);
        switchSGear(up);
        sendCommand();
    }
    else if (SEQUENCIAL == 1 && buttonStates[16] > 320 && buttonStates[16] < 680)
    {
        G27.setButton(19, LOW);
        G27.setButton(20, LOW);
        setHMode(false);
        switchSGear(center);
        sendCommand();
    }
